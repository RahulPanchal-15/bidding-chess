...
contract("Simulate Game:", (accounts) => {
  ...
  it(" W3 moves Qh5 - Bids (min*10)wei ", async() => {
    let factory = await Factory.deployed();
    let gameAddress = await factory.getLatestGame(); 
    let game = await Game.at(gameAddress);
    await truffleAssert.passes(
  ...
  it(" B3 moves Nf6 - Bids (min)wei ", async() => {
    let factory = await Factory.deployed();
    let gameAddress = await factory.getLatestGame(); 
    let game = await Game.at(gameAddress);
    let tx = null;
  ...
  it(" W4 moves Qxf7# - Bids (min*10)wei ", async() => {
    ...
    let w1 = await game.playerContribution(W1);
    let w3 = await game.playerContribution(W3);

    console.log("W1 Contribution : ", w1.toString());
    console.log("W3 Contribution : ", w3.toString());
    ...
    console.log("LatestGame : ",await factory.getLatestGame());

    console.log("ChessFatory UBI: ", await ubi.balanceOf(Factory.address));
